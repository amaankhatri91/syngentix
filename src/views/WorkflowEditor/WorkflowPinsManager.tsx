...
interface WorkflowPinsManagerProps {
  pins: NodePin[];
...
const WorkflowPinsManager: React.FC<WorkflowPinsManagerProps> = ({
  pins,
  onPinsChange,
  title,
  pinType,
  ...
  const handleSaveEdit = (index: number) => {
    if (editingName.trim()) {
      const pinToUpdate = localPins[index];
      ...
      // Emit pin:update event if it's a custom pin
      if (pinToUpdate?.custom && workflowId && nodeId && pinToUpdate.id) {
        ...
        const pinCollection = pinCollectionMap[pinType] || "inputs";
        ...
        const updatePayload = {
          ...
          pin: {
            id: pinToUpdate.id,
            name: editingName.trim(),
            type: pinToUpdate.type || "any",
            required: pinToUpdate.required || false,
          },
        };
        
        console.log("Pin update payload:", updatePayload);
        emit("pin:update", updatePayload);
      }
  ...
  const handleDeletePin = (index: number) => {
    ...
    // Only emit delete event if we have required data and it's a custom pin
    if (pinToDelete?.custom && workflowId && nodeId && pinToDelete.id) {
      // Map pinType to pin_collection
      const pinCollectionMap: Record<string, string> = {
        input: "inputs",
        output: "outputs",
      ...
      const deletePayload = {
        ...
        node_id: nodeId,
        pin_collection: pinCollection,
        pin_id: pinToDelete.id,
      };
      
      console.log("Pin delete payload:", deletePayload);
      emit("pin:delete", deletePayload);
    }
...
export default WorkflowPinsManager;
