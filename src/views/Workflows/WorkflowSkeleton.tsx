  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className={`${isDark ? "bg-[#0F1724]" : "bg-white"} ${
            isDark ? "border-[#2B3643]" : "border-[#EEF4FF]"
          } border rounded-2xl px-4 py-3 flex flex-wrap items-center gap-y-4 transition-all duration-200`}
          style={
            !isDark
              ? {
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 basis-full md:basis-6/12 xl:basis-4/12">
            {[1, 2, 3, 4].map((item) => (
              <div
                key={item}
                className={`${isDark ? "bg-[#0C1116]" : "bg-[#F5F7FA]"} ${
                  isDark ? "border-[#394757]" : "border-[#E3E6EB]"
                } border h-[36px] px-3 rounded-lg flex items-center gap-1 animate-pulse`}
